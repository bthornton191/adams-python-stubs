...
The `docs/` directory (gitignored) is generated by MkDocs from the stubs.
...
The script creates the necessary `.py` discovery shims, runs `mkdocs build`,
then deletes the shims automatically — even if the build fails.

The built site lands in `docs/`. Open `docs/index.html` directly in a browser —
no web server needed.

To run `mkdocs serve` for live preview instead:

```bash
python scripts/docs_setup.py -- serve
```

> **Why the shim step?** Griffe (the static analysis backend) only discovers
> Python modules via `.py` files. It does not find bare `.pyi` stub files on
> its own. `docs_setup.py` creates an empty `.py` alongside each `.pyi` so
> griffe can locate the module and then load the `.pyi` as its stubs companion.
> The shims are deleted after the build.

> **Note for maintainers:** If you add a new `.pyi` file, also add its
> corresponding `.py` entry to `.gitignore`.
