...
4. **Docstrings** — Add docstrings to non-obvious methods and enumerations. Use the numpy docstring style already present in the repo. Include `Example` sections showing real usage.
5. **Enums as `Literal`** — Where a string parameter has a fixed set of valid values, prefer `Literal['option1', 'option2', ...]` over bare `str`.
